...
public class TextPostDBRepository implements Repository<Long, TextPost> {
     ...
     */
    public Iterable<TextPost> findAll(Long idUser, ContentPage currentPage, PostUserType postUserType) {
    ...
    private String postUserTypeToCommand(Long idUser, ContentPage currentPage, PostUserType postUserType) {
        ...
        if (postUserType == PostUserType.USER) {
            return "SELECT * FROM \"textPosts\" WHERE \"UserID\" = " + idUser + " ORDER BY \"Date\" DESC" + limitQuery;
        }
        // PostUserType.FRIEND
        return "SELECT * FROM \"textPosts\" WHERE \"UserID\" IN " + subQuery + " ORDER BY \"Date\" DESC" + limitQuery;
    }
}
